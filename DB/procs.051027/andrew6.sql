...
BEGIN
  /* PROCEDURE BODY */
  FOR
   SELECT LOCN_ID,ISSN.PROD_ID,CURRENT_QTY,PROD_PROFILE.SHORT_DESC, ISSN.ISSN_STATUS AS THISSTATUS
...
BEGIN
  /* PROCEDURE BODY */
  --- IF MODE 0 THEN ORDER BY PRODUCTID, RECEIVE DATE
  --- IF MODE 1 THEN ORDER BY GRN, PRODUCTID
  FOR
  ...
  WHERE TRN_TYPE = 'GRNV' AND TRN_CODE = 'P' AND PROD_PROFILE.PROD_ID = :THISPROD_ID
--- AND  (TRN_DATE >= :THISSTART AND TRN_DATE <= :THISEND) AND
  INTO
:PROD,
:QTY,
:TRN_DATE,
