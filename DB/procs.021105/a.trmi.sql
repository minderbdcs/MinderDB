...
BEGIN
  ...
  BEGIN       
     ...
     */
     ...
     str_mobile_LOCN_ID = substr(SUBLOCN_ID, 3,8);
     IF (LOCN_ID = str_mobile_LOCN_ID) THEN
     BEGIN
        EXECUTE PROCEDURE UPDATE_TRAN (:RECORD_ID, 'F', 'Tried to move mobile location within itself');
        EXIT;
     END
     /* must check that we are not moving this down the tree */
     ...
     BEGIN
        ...
        BEGIN
           ...
           IF (str_last_parent_LOCN_ID = LOCN_ID) THEN
           BEGIN
              EXECUTE PROCEDURE UPDATE_TRAN (:RECORD_ID, 'F', 'Tried to move mobile location further down the tree');
              EXIT;
           END
        END
     END
     ...
     BEGIN
        ...
        IF (strWH_ID <> str_mobile_WH_ID) THEN
        BEGIN
           UPDATE SSN
              SET WH_ID = :strWH_ID      
