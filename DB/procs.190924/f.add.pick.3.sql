...
CREATE OR ALTER PROCEDURE ADD_PICK_ITEMS (WK_ORDER PICK_ORDER,
WK_ORDER_STATUS CHAR(1) ,
WK_STATUS VARCHAR(6) ,
WK_PROD1 PRODUCT,
WK_EXTENSION1 VARCHAR(6) ,
...
DECLARE VARIABLE WK_SERVER_VERSION VARCHAR(100) ;
DECLARE VARIABLE WK_COMPANY_ALL VARCHAR(50) ;
BEGIN
   ...
   WK_LOG_FILENAME = '/data/tmp/addpickprod.log';
...
/*    WK_LINE_NO1 = ALLTRIM(WK_LINE_NO1); */
      WK_LINE_NO1 = TRIM(WK_LINE_NO1);
/*    WK_LINE_LEN = STRLEN(WK_LINE_NO1); */
      WK_LINE_LEN = CHAR_LENGTH(WK_LINE_NO1);
      WHILE (WK_LINE_LEN < 4)
      DO
      BEGIN
      ...
      END
   END
   /* that gives '   0' line_no */
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'line: ' || :WK_LINE_NO1 || ':');
   WK_COMPANY_ALL = 'F';
   SELECT DESCRIPTION  
   FROM OPTIONS
   WHERE GROUP_CODE = 'TRANSFER'
   AND CODE = 'ONE-COMPANY'
   INTO :WK_COMPANY_ALL;
...
/* IF (STRLEN(WK_PROD1) <= :WK_PROD_EAN_LENGTH) THEN */
   IF (CHAR_LENGTH(WK_PROD1) <= :WK_PROD_EAN_LENGTH) THEN
   BEGIN
   ...
   BEGIN
      SELECT FIRST 1 PROD_TYPE , SALE_PRICE 
      FROM PROD_PROFILE
      WHERE PROD_ID = :WK_PROD_ID
   ...
   WK_SERVER_VERSION = '1.0';    
   SELECT RDB$GET_CONTEXT('SYSTEM', 'ENGINE_VERSION')
   FROM CONTROL 
   INTO :WK_SERVER_VERSION;    
/*
...
/*    IF (WK_LINE_NO1 IS NULL OR WK_LINE_NO1 = '' OR (WK_LINE_NO1 = '   0' )) THEN */
   ...
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'line empty or 0 '  );
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'order: ' || :WK_ORDER );
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'prod: ' || :WK_PROD_ID);
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'line: ' || :WK_LINE_NO1 || ':');
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'found:' || :WK_FOUND || ' labelno: '  );
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PICK_LABEL  );
      END
      ELSE
      BEGIN
         /*  AND (PICK_LINE_STATUS IN ('UC','CF','OP','UP') */
   ...
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'label: ' || :WK_PICK_LABEL);
      ...
      BEGIN
         ...
         INTO :WK_PICK_LABEL;
/*       IF ((WK_LINE_NO1 > '') AND (WK_LINE_NO1 <> '0') AND (WK_LINE_NO1 <> '   0')) THEN */
         IF ((WK_LINE_NO1 > '') AND (WK_LINE_NO1 <> '0') AND (WK_LINE_NO1 <> '0   ')) THEN
         BEGIN
         ...
         VALUES (
            ...
            :WK_PI_LINE_TOTAL );
         WK_LINE_ADDED = 'T';
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'inserted pick_item: ' || :WK_PICK_LABEL);
      END
      ELSE
      BEGIN
         UPDATE PICK_ITEM 
   ...
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pick_item: ' || :WK_PICK_LABEL);
      END
   END
   ELSE
   BEGIN
      ...
      BEGIN
         ...
         SET PICK_RETRIEVE_STATUS = :WK_STATUS,
            ...
            USER_ID = :WK_PERSON_ID
         WHERE PICK_ORDER = :WK_ORDER;
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pick_item rectrieve status false: ' || :WK_PROD_ID);
      END
      ELSE
      BEGIN
         /* the line is false*/
         ...
         BEGIN
            ...
            SET PICK_RETRIEVE_STATUS = :WK_STATUS,
               ...
               USER_ID = :WK_PERSON_ID
            WHERE PICK_LABEL_NO = :WK_PICK_LABEL;
   WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pick_item retrive status false: ' || :WK_PICK_LABEL);
         END
      END
   END
