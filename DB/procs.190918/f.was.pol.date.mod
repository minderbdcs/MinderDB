COMMIT WORK;
SET AUTODDL OFF;
SET TERM ^ ;
 
CREATE OR ALTER TRIGGER MOD_PURCHASE_LINE_DATE FOR PURCHASE_ORDER_LINE 
ACTIVE BEFORE UPDATE POSITION 2 
AS
DECLARE VARIABLE WK_PO_STATUS CHAR(2);
DECLARE VARIABLE WK_PO_ORDER PURCHASE_ORDER;
DECLARE VARIABLE WK_POL_LINE CHAR(4);
DECLARE VARIABLE WK_MORE_LINES CHAR(4);
DECLARE VARIABLE WK_PROD_ID     PRODUCT;
DECLARE VARIABLE WK_FOUND INTEGER;
DECLARE VARIABLE WK_OLD_NOTE BLOB ;
DECLARE VARIABLE WK_NOTE_STR2 VARCHAR(32000);
DECLARE VARIABLE WK_OLD_STR VARCHAR(32000);
DECLARE VARIABLE WK_START_POSN INTEGER;
DECLARE VARIABLE WK_END_POSN INTEGER;
DECLARE VARIABLE WK_LEN_TOGET INTEGER;
DECLARE VARIABLE WK_ORDER_STATUS VARCHAR(40);
DECLARE VARIABLE WK_PO_COMPANY_ID COMPANY;
DECLARE VARIABLE WK_CN_COMPANY_ID COMPANY;
DECLARE VARIABLE WK_DO_CHART CHAR(1);
DECLARE VARIABLE WK_PROD_LEDGER_CODE VARCHAR(50);
DECLARE VARIABLE WK_DUMMY INTEGER;
BEGIN
   NEW.LAST_UPDATE_DATE = 'NOW';
   IF ((NEW.PO_LINE_QTY IS NOT NULL) AND (NEW.ORIGINAL_QTY IS NULL)) THEN
   BEGIN
      NEW.ORIGINAL_QTY = NEW.PO_LINE_QTY;
   END
   WK_CN_COMPANY_ID = NULL;
   WK_ORDER_STATUS = '';
   SELECT DEFAULT_PURCHASE_STATUS, COMPANY_ID
   FROM CONTROL
   INTO :WK_ORDER_STATUS , :WK_CN_COMPANY_ID;
   IF (NEW.PO_LINE_STATUS IS NULL) THEN
   BEGIN
      NEW.PO_LINE_STATUS = :WK_ORDER_STATUS;
   END
   IF (NEW.PO_LINE_STATUS = '') THEN
   BEGIN
      NEW.PO_LINE_STATUS = :WK_ORDER_STATUS;
   END
   WK_PO_ORDER = NEW.PURCHASE_ORDER;
   /* check order status */
   IF (NEW.PO_LINE_STATUS = 'CL' OR
       NEW.PO_LINE_STATUS = 'OS')  THEN
   BEGIN
      /* get the orders po_status */
      WK_PO_STATUS = '';
      WK_PO_ORDER = NEW.PURCHASE_ORDER;
      WK_POL_LINE = NEW.PO_LINE;
      SELECT PO_STATUS
      FROM PURCHASE_ORDER
      WHERE PURCHASE_ORDER = :WK_PO_ORDER
      INTO :WK_PO_STATUS;
      IF (WK_PO_STATUS = 'OP' OR
          WK_PO_STATUS = 'CF') THEN
      BEGIN
         /* are there any cf or op lines on this order */
         WK_MORE_LINES = '';
         SELECT FIRST 1 PO_LINE
         FROM PURCHASE_ORDER_LINE
         WHERE PURCHASE_ORDER = :WK_PO_ORDER
         AND PO_LINE <> :WK_POL_LINE
         AND PO_LINE_STATUS IN ('OP','CF')
         INTO :WK_MORE_LINES;
         IF (WK_MORE_LINES IS NULL) THEN
         BEGIN
            WK_MORE_LINES = '';
         END
         IF (WK_MORE_LINES = '') THEN
         BEGIN
            /* update purchase_order.po_status to 'CL' */
            UPDATE PURCHASE_ORDER
            SET PO_STATUS = 'CL'
            WHERE PURCHASE_ORDER = :WK_PO_ORDER;
         END
      END
   END
   /* check that the prod exists */
   WK_PROD_ID = NEW.PROD_ID;
/* what company */
   WK_PO_COMPANY_ID = NULL;
   SELECT COMPANY_ID
   FROM PURCHASE_ORDER
   WHERE PURCHASE_ORDER = :WK_PO_ORDER
   INTO :WK_PO_COMPANY_ID;
   WK_FOUND = 0;
   SELECT 1 FROM PROD_PROFILE
   WHERE PROD_ID = :WK_PROD_ID
   AND COMPANY_ID = :WK_PO_COMPANY_ID
   INTO :WK_FOUND;
   IF (WK_FOUND IS NULL) THEN
   BEGIN
      WK_FOUND = 0;
   END
   IF (WK_FOUND = 0) THEN
   BEGIN
      IF (WK_PO_COMPANY_ID IS NULL) THEN
      BEGIN
         /* try default company */
         SELECT 1 FROM PROD_PROFILE
         WHERE PROD_ID = :WK_PROD_ID
         AND COMPANY_ID = :WK_CN_COMPANY_ID
         INTO :WK_FOUND;
         IF (WK_FOUND IS NULL) THEN
         BEGIN
            WK_FOUND = 0;
         END
      END
   END
   IF (WK_FOUND = 0) THEN
   BEGIN
      WK_OLD_NOTE = NEW.PO_LINE_DESCRIPTION;
      IF (WK_OLD_NOTE IS NULL) THEN
      BEGIN
         WK_NOTE_STR2 = NEW.PROD_ID;
      END 
      ELSE
      BEGIN
         WK_START_POSN = 1;
         WK_END_POSN = BLOB_BYTECOUNT(:WK_OLD_NOTE);
         WK_LEN_TOGET = WK_END_POSN - WK_START_POSN + 1;
/* use 250 rather than 50 */
         IF (WK_LEN_TOGET > 250) THEN
         BEGIN
            WK_LEN_TOGET = 250;
         END
/*
         WK_NOTE_STR2 = BLOB_SUBSTR(:WK_OLD_NOTE, :WK_START_POSN, :WK_START_POSN + :WK_LEN_TOGET);
         WK_NOTE_STR2 = V6SUBSTRING(WK_OLD_STR,WK_START_POSN, WK_START_POSN + :WK_LEN_TOGET);
*/
         WK_OLD_STR = CAST (WK_OLD_NOTE AS VARCHAR(32000));
         WK_NOTE_STR2 = LEFT(WK_OLD_STR, :WK_LEN_TOGET);
      END
      INSERT INTO PROD_PROFILE (PROD_ID,SHORT_DESC, LONG_DESC, COMPANY_ID) VALUES (NEW.PROD_ID, :WK_NOTE_STR2, :WK_NOTE_STR2, :WK_PO_COMPANY_ID);
   END

   WK_DO_CHART = 'F';
   SELECT LEGACY_LEDGER_ACCOUNTS 
   FROM CONTROL
   INTO :WK_DO_CHART;
   IF (WK_DO_CHART IS NULL) THEN
   BEGIN
      WK_DO_CHART = 'F';
   END

   IF (WK_DO_CHART = 'T') THEN
   BEGIN
      IF ((NEW.LEGACY_LEDGER_PURCHASE_CODE IS NULL) OR (NEW.LEGACY_LEDGER_PURCHASE_CODE = '')) THEN
      BEGIN
         /* no purchase account code */
         /* so 1st try one for the product */
         WK_FOUND = 0;
         WK_PROD_LEDGER_CODE  = NULL;
         IF ((NEW.PROD_ID IS NOT NULL) AND (NEW.PROD_ID <> '')) THEN
         BEGIN
            WK_FOUND = 0;
            SELECT 1, LEGACY_LEDGER_PURCHASE_CODE
            FROM PROD_PROFILE
            WHERE PROD_PROFILE.PROD_ID = NEW.PROD_ID
            AND COMPANY_ID = :WK_PO_COMPANY_ID
            INTO :WK_FOUND, :WK_PROD_LEDGER_CODE ;
            IF (WK_FOUND IS NULL) THEN
            BEGIN
               WK_FOUND = 0;
            END
            IF (WK_FOUND = 0) THEN
            BEGIN
               /* try all */
               SELECT  LEGACY_LEDGER_PURCHASE_CODE
               FROM PROD_PROFILE
               WHERE PROD_PROFILE.PROD_ID = NEW.PROD_ID
               AND COMPANY_ID = 'ALL'
               INTO :WK_PROD_LEDGER_CODE ;
            END
         END
         IF ((WK_PROD_LEDGER_CODE IS NULL) OR (WK_PROD_LEDGER_CODE = '')) THEN
         BEGIN
            WK_DUMMY = 1;
         END
         ELSE
         BEGIN
            NEW.LEGACY_LEDGER_PURCHASE_CODE = WK_PROD_LEDGER_CODE;
         END
      END
   END
END ^


SET TERM ; ^
COMMIT WORK;
SET AUTODDL ON;
