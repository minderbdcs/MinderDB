...
AS 
  ...
  DECLARE VARIABLE WK_IS_ZONE_C VARCHAR(4);
  ...
  DECLARE VARIABLE WK_ZONE_C VARCHAR(4);
...
BEGIN
   ...
   CNT = 0;
   /* 13/11/12 use default wh from control
               use list of companys in company table
   */
   ...
   /* op to al - over ordered pick item lines */   
      ...
      WK_IS_ZONE_C = 'NONE';
      FOR SELECT COMPANY.COMPANY_ID, COMPANY.MINDER_WH_ID
      FROM COMPANY 
      WHERE COMPANY.MINDER_WH_ID IS NOT NULL
      INTO :WK_IS_COMPANY_ID, :WK_IS_WH_ID
      DO
      BEGIN
         WK_WH_ID = WK_IS_WH_ID;
         WK_COMPANY_ID = WK_IS_COMPANY_ID;
         FOR SELECT ZONE.CODE
         FROM ZONE    
         WHERE ZONE.COMPANY_ID = :WK_COMPANY_ID  
         INTO :WK_IS_ZONE_C
         DO
         BEGIN
            WK_ZONE_C = WK_IS_ZONE_C;
            ...
            /* FROM PRODUCT_CMP_WH_STOCK_STATUS_V('%',:WK_COMPANY_ID,:WK_WH_ID,'Admin','|PROD_ID|AVAILABLE_QTY|UNPICKED_ORDER_QTY|') AS P1  */
            FOR SELECT P1.PROD_ID,P1.AVAILABLE_QTY, P1.UNPICKED_ORDER_QTY
            FROM PRODUCT_CMP_WH_STOCK_STATUS_6('%',:WK_COMPANY_ID,:WK_WH_ID,:WK_ZONE_C, 'Admin','|PROD_ID|AVAILABLE_QTY|UNPICKED_ORDER_QTY|') AS P1  
            ...
            BEGIN
               ...
               FROM  PICK_ITEM
               JOIN PICK_ORDER ON PICK_ORDER.PICK_ORDER = PICK_ITEM.PICK_ORDER AND PICK_ORDER.PICK_STATUS IN  ('OP','DA')  AND PICK_ORDER.COMPANY_ID = :WK_IS_COMPANY_ID  AND PICK_ORDER.WH_ID = :WK_IS_WH_ID AND PICK_ORDER.ZONE_C = :WK_IS_ZONE_C
               ...
               ORDER BY  PICK_ORDER.PICK_DUE_DATE , PICK_ORDER.PICK_PRIORITY
         ...
         */
         ...
         BEGIN
            ...
            BEGIN
               ...
               FOR SELECT PICK_ITEM.PICK_LABEL_NO, (COALESCE(PICK_ITEM.PICK_ORDER_QTY, 0) - COALESCE(PICK_ITEM.PICKED_QTY,0)) AS  TOPICK_QTY, PICK_ORDER.PARTIAL_PICK_ALLOWED
               FROM  PICK_ITEM
               JOIN PICK_ORDER ON PICK_ORDER.PICK_ORDER = PICK_ITEM.PICK_ORDER AND PICK_ORDER.PICK_STATUS IN  ('OP','DA')  AND PICK_ORDER.COMPANY_ID = :WK_IS_COMPANY_ID  AND PICK_ORDER.WH_ID = :WK_IS_WH_ID  
               WHERE PICK_ITEM.PROD_ID = :WK_IS_PROD_ID 
               AND   PICK_ITEM.PICK_LINE_STATUS = 'OP'
               AND PICK_ORDER.ZONE_C IN ( NULL, 'AZ')
               /* ORDER BY  PICK_ORDER.PICK_PRIORITY, PICK_ORDER.PICK_DUE_DATE */
               ORDER BY  PICK_ORDER.PICK_DUE_DATE , PICK_ORDER.PICK_PRIORITY
               INTO :WK_PI_LABEL_NO, :WK_PI_TOPICK_QTY, :WK_PO_PARTIAL_PICK
               DO
   ...
   /* al to op - have some stock pick item lines */   
      ...
      BEGIN
         ...
         BEGIN
            ...
            JOIN PICK_ORDER P3 ON P3.PICK_ORDER = P2.PICK_ORDER AND P3.PICK_STATUS IN  ('OP','DA')  AND P3.COMPANY_ID = :WK_IS_COMPANY_ID  AND P3.WH_ID = :WK_IS_WH_ID AND P3.ZONE_C = :WK_IS_ZONE_C
         ...
         BEGIN
            ...
            FROM PRODUCT_CMP_WH_STOCK_STATUS_6('%',:WK_COMPANY_ID,:WK_WH_ID, :WK_ZONE_C, 'Admin','|PROD_ID|AVAILABLE_QTY|UNPICKED_ORDER_QTY|') AS P1  
            JOIN PICK_ITEM P2 ON P2.PROD_ID = P1.PROD_ID AND P2.PICK_LINE_STATUS = 'AS' AND COALESCE(P2.PICK_ORDER_QTY,0) > COALESCE(P2.PICKED_QTY,0) 
            JOIN PICK_ORDER P3 ON P3.PICK_ORDER = P2.PICK_ORDER AND P3.PICK_STATUS IN  ('OP','DA')  AND P3.COMPANY_ID = :WK_IS_COMPANY_ID  AND P3.WH_ID = :WK_IS_WH_ID AND P3.ZONE_C IN ( NULL, 'AZ')         
            WHERE P1.AVAILABLE_QTY > P1.UNPICKED_ORDER_QTY AND P1.AVAILABLE_QTY > 0
            INTO :WK_PI_LABEL_NO 
            DO
            BEGIN
   ...
   /* al to op - have some stock pick item lines */   
      ...
      BEGIN
         ...
         BEGIN
            ...
            /* FROM PRODUCT_CMP_WH_STOCK_STATUS_V('%',:WK_COMPANY_ID,:WK_WH_ID,'Admin','|PROD_ID|AVAILABLE_QTY|') AS P1  */
            FOR SELECT P2.PICK_LABEL_NO
            FROM PRODUCT_CMP_WH_STOCK_STATUS_6('%',:WK_COMPANY_ID,:WK_WH_ID, :WK_ZONE_C, 'Admin','|PROD_ID|AVAILABLE_QTY|') AS P1  
         ...
         BEGIN
            ...
            FROM PRODUCT_CMP_WH_STOCK_STATUS_6('%',:WK_COMPANY_ID,:WK_WH_ID, :WK_ZONE_C, 'Admin','|PROD_ID|AVAILABLE_QTY|') AS P1  
            JOIN PICK_ITEM P2 ON P2.PROD_ID = P1.PROD_ID AND P2.PICK_LINE_STATUS = 'OP' AND COALESCE(P2.PICK_ORDER_QTY,0) > COALESCE(P2.PICKED_QTY,0) 
            JOIN PICK_ORDER P3 ON P3.PICK_ORDER = P2.PICK_ORDER AND P3.PICK_STATUS IN  ('OP','DA')  AND P3.COMPANY_ID = :WK_IS_COMPANY_ID  AND P3.WH_ID = :WK_IS_WH_ID 
           AND P3.ZONE_C IN ( NULL, 'AZ')
            WHERE P1.AVAILABLE_QTY = 0
            INTO :WK_PI_LABEL_NO 
            DO
            BEGIN
