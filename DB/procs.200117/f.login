...
DECLARE VARIABLE WK_ALLOW_DHCP_REUSE VARCHAR(50);
DECLARE VARIABLE WK_DHCP_DEVICE INTEGER;
...
BEGIN
    ...
    BEGIN
        ...
        IF (WK_IP > '') THEN
        BEGIN
/* check that we allow this - this means only 1 device for all connection from the ip address !!!!! */
            WK_ALLOW_DHCP_REUSE = 'F';
            SELECT DESCRIPTION FROM OPTIONS WHERE GROUP_CODE = 'LOGIN' AND  CODE = 'DHCP-REUSE' INTO :WK_ALLOW_DHCP_REUSE;
            IF (WK_ALLOW_DHCP_REUSE = 'T') THEN
            BEGIN
               ...
               WHERE SYS_EQUIP.IP_ADDRESS = 'DHCP'
                 ...
                 AND SESSION.LAST_UPDATE_DATE > 'TODAY'
               INTO :DEVICE_ID
               DO
               BEGIN
                   LG_STATUS = 24;
        ...
        BEGIN
            ...
            BEGIN     
                ...
                BEGIN
                    ...
                    IF (WK_DHCP_DEVICE < 0) THEN
                    BEGIN
                        UPDATE SYS_EQUIP SET SYS_EQUIP.WH_ID = (SELECT SYS_USER.DEFAULT_WH_ID FROM SYS_USER WHERE SYS_USER.USER_ID = :WK_USER )
                        WHERE SYS_EQUIP.DEVICE_ID = :DEVICE_ID;
                    END
