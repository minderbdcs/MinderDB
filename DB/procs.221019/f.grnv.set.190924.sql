...
CREATE OR ALTER PROCEDURE ADD_SSN_ISSN_GRNV_SET (WH_ID WH_ID ,
LOCN_ID LOCN_ID ,
OBJECT OBJECT ,
...
SOURCE VARCHAR(9) ,
RECORD_ID INTEGER,
WK_SSNQTY4 VARCHAR(10),
WK_LABELQTY4 VARCHAR(10),
W_PRODUCT_SSN_ID SSN_ID,
IN_RESPONSE_TXT ERROR_TEXT,
IN_RESPONSE_TXT2 ERROR_TEXT)
RETURNS (
   OUT_RESPONSE_TXT ERROR_TEXT,
   OUT_RESPONSE_TXT2 ERROR_TEXT ,
   WK_HAVE_FILE_2 INTEGER )
AS 
...
DECLARE VARIABLE WK_ALTPROD PRODUCT;
...
/* WK_REF_LEN = STRLEN(REFERENCE); */
   ...
   IF (WK_TRN_TYPE IS NULL) THEN
   BEGIN
      WK_TRN_TYPE = 'GRNV';
   END
   IF (WK_TRN_CODE IS NULL) THEN
   BEGIN
      WK_TRN_CODE = '';
   END
   ...
   SELECT GENERATE_LABEL_TEXT , PROD_PROFILE_STD_COST_UPDATE, COALESCE(DEFAULT_PROD_TYPE,'UNKNOWN')
   FROM CONTROL 
   INTO :WK_GENERATE_LABEL_TEXT, :WK_CN_STD_COST_UPDATE, :WK_CN_PROD_TYPE;
...
-* WHILE ((SUBSTR(REFERENCE,WK_REF2_LAST,WK_REF2_LAST) <> '|') AND *-
   WHILE ((SUBSTRING(REFERENCE FROM WK_REF2_LAST FOR 1) <> '|') AND
          (WK_OLD_METHOD = 'Y')) DO
   BEGIN
-*    IF (SUBSTR(REFERENCE,WK_REF2_LAST,WK_REF2_LAST) < '0' OR
          SUBSTR(REFERENCE,WK_REF2_LAST,WK_REF2_LAST) > '9') THEN *-
      IF (SUBSTRING(REFERENCE FROM WK_REF2_LAST FOR 1) < '0' OR
          SUBSTRING(REFERENCE FROM WK_REF2_LAST FOR 1) > '9') THEN
      BEGIN      
         WK_OLD_METHOD = 'N';
      END
...
/* WK_DATE = EXTRACT(DAY FROM TRANS_DATE) || '/' || EXTRACT(MONTH FROM TRANS_DATE) || '/' || SUBSTR(CAST(EXTRACT(YEAR FROM TRANS_DATE) AS CHAR(4)) , 3,4); */
   ...
   SELECT WORKING_DIRECTORY FROM SYS_EQUIP 
       ...
       INTO :WK_DIRECTORY;
   OUT_RESPONSE_TXT = :IN_RESPONSE_TXT  ;   
   OUT_RESPONSE_TXT2 = :IN_RESPONSE_TXT2 ;
   IF (OUT_RESPONSE_TXT IS NULL) THEN
   BEGIN
       OUT_RESPONSE_TXT = '';
   END
   IF (OUT_RESPONSE_TXT2 IS NULL) THEN
   BEGIN
       OUT_RESPONSE_TXT2 = '';
   END
/* ================================================================================================================================ 
...
OUT_RESPONSE_TXT2
   ...
   LABEL_NO = CAST(WK_SSNQTY4 AS INTEGER);
   WK_LABEL_CURQTY = CAST(WK_LABELQTY4 AS INTEGER);
   WK_A_PROD = 0;
   WK_A_PO = 0;
...
either ALL or the company for this GRN */
           ...
           SELECT 1, SHORT_DESC , HOME_LOCN_ID, ALTERNATE_ID, PALLET_CFG_INNER, PALLET_CFG_ALTERNATE,
              ISSUE_UOM, ISSUE_PER_ORDER_UNIT, ISSUE_PER_INNER_UNIT, SPECIAL_INSTR, TEMPERATURE_ZONE,
              SSN_TYPE, PROD_TYPE, COMPANY_ID, STANDARD_COST
              FROM PROD_PROFILE
              ...
              :WK_TEMPERATURE_ZONE, :WK_SSN_TYPE, :WK_PROD_TYPE, :WK_PROD_OWNER, :WK_PROD_STD_COST;
...
/*             WK_OWNER = ALLTRIM(WK_OWNER); */
               WK_OWNER = TRIM(WK_OWNER);
/*             WK_PROD_OWNER = ALLTRIM(WK_OWNER); */
...
/*         WK_OWNER = ALLTRIM(WK_OWNER); */
		 ...
		 /* calc wh_adj_units = qty to create in grnv */
                 WK_ADJ_UNITS  = LABEL_NO  * WK_LABEL_CURQTY ;
                 WK_ADJ_UNITS  = WK_ADJ_UNITS + (LABEL_NO2  * WK_LABEL_CURQTY2) ;
/*
                 EXECUTE PROCEDURE ADD_PRODUCT_COST (:WH_ID, :WK_LOADNO, :WK_LOAD_LINE_NO, :OBJECT, 
                                                     :WK_ADJ_UNITS, :TRANS_DATE, :WK_USER, :RECORD_ID);
*/
              END
           END
       END
...
/* need company for select either ALL or the company for this GRN */
           ...
           SELECT 1, PROD_TYPE, COMPANY_ID, SHORT_DESC , HOME_LOCN_ID, ALTERNATE_ID,
              ...
              SPECIAL_INSTR, TEMPERATURE_ZONE, SSN_TYPE
              FROM PROD_PROFILE
              WHERE PROD_ID = :OBJECT
              AND COMPANY_ID = :WK_OWNER
              INTO :WK_PROD_EXISTS, :WK_PROD_TYPE, :WK_PROD_OWNER, :WK_PROD_DESC, :WK_PUTLOCN_1,
              :WK_ALTPROD, :WK_STD_PALLET_CFG, :WK_NON_PALLET_CFG, :WK_ISSUE_UOM, :WK_ORDER_UNITS,
              :WK_INNER_UNITS, :WK_INSTR, :WK_TEMPERATURE_ZONE, :WK_SSN_TYPE;
           IF (WK_PROD_EXISTS = 0) THEN
           BEGIN
              SELECT 1, PROD_TYPE, COMPANY_ID, SHORT_DESC , HOME_LOCN_ID, ALTERNATE_ID,
                 PALLET_CFG_INNER, PALLET_CFG_ALTERNATE, ISSUE_UOM, ISSUE_PER_ORDER_UNIT, ISSUE_PER_INNER_UNIT,
...
/*       SELECT ISSN_ID FROM GET_ISSN_BARCODE('BARCODE', :P_SSN_ID, 36) INTO :I_SSN_ID; */
         ...
         BEGIN
            /* Insert data into SSN table */   
            /* not a prod so use the system default prod type for the ssn_type */
            ...
            VALUES (:I_SSN_ID, :I_SSN_ID, :WH_ID, :LOCN_ID, :WK_LABEL_CURQTY, :WK_STATUS, 'NOW', :WK_OWNER, :WK_LABEL_CURQTY, :TRANS_DATE, :WK_USER, :TRANS_DATE, :WH_ID); 
             BEGIN
         ...
         BEGIN
            ...
            VALUES (:I_SSN_ID, :W_PRODUCT_SSN_ID, :WH_ID, :LOCN_ID, :WK_LABEL_CURQTY, :WK_PROD_STATUS, :OBJECT, 'NOW', :WK_OWNER, :WK_LABEL_CURQTY, :TRANS_DATE, :WK_USER, :TRANS_DATE, :WH_ID); 
             BEGIN
                /* now add transactions archive for this ssn_id */
                INSERT INTO TRANSACTIONS_ARCHIVE
                  (WH_ID, LOCN_ID, OBJECT, TRN_TYPE, TRN_CODE, TRN_DATE, REFERENCE,
...
/*            SELECT MAX(LAST_PALLET_NO) FROM GRN WHERE LAST_LOT_NO STARTING SUBSTR(:WK_GRN_LOT,1,8) INTO :WK_LAST_LOT_PALLET_NO; */
   ...
   END /* if ssn method is 'CMP=2|GRN=6|LINE=2|SFX=2' */
/* =================================================================================================================== */
/*   OUT_RESPONSE_TXT = :IN_RESPONSE_TXT || :WK_1ST_ISSN || '|' || :WK_SSNQTY4 || '|' || :WK_LABELQTY4 || '|' || :WK_PRINTER || '|' ;  */
   OUT_RESPONSE_TXT = :IN_RESPONSE_TXT || :WK_1ST_ISSN || '|' || :WK_SSNQTY4 || '|' || :WK_LABELQTY4 || '|'  ;   
   OUT_RESPONSE_TXT2 = '|' || :WK_1ST_ISSN || '|' || :WK_SSNQTY4 || '|' || :WK_LABELQTY4 ;
