...
AS
  ...
  DECLARE VARIABLE WK_SSN_PICK_ORDER PICK_ORDER;
...
BEGIN
   ...
   WK_USER = NEW.USER_ID;
   IF (WK_NEW_WH_ID >= 'X' AND WK_NEW_WH_ID <= 'X~') THEN
   BEGIN
/*    IF (LEN(WK_NEW_LOCN_ID) = 2) THEN */
      IF (CHAR_LENGTH(WK_NEW_LOCN_ID) = 2) THEN
      BEGIN
         WK_NEW_LOCN_ID = '00000000';
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  IF ((NEW.CURRENT_QTY IS NULL) OR (NEW.CURRENT_QTY = 0)) THEN
                  BEGIN
                     IF ((NEW.PREV_QTY IS NOT NULL) AND (NEW.PREV_QTY > 0)) THEN
                     BEGIN
                        NEW.CURRENT_QTY = NEW.PREV_QTY; 
                     END
                  END
               END
            END
            IF (WK_FROM_STATUS = 'DX') THEN
            BEGIN
               /* move from Despatched 
                  so remove from confimation of previous order */
               NEW.PREV_PREV_PICK_ORDER = OLD.PREV_PICK_ORDER;
               NEW.PREV_PICK_ORDER = OLD.PICK_ORDER;
               NEW.PICK_ORDER = NULL;
            ...
            IF (WK_TO_STATUS = 'ST') THEN
            BEGIN
               IF ((NEW.OTHER1 = NEW.SERIAL_NUMBER) AND
                   (NEW.OTHER1 IS NOT NULL) AND
                   (NEW.SERIAL_NUMBER IS NOT NULL)) THEN
               BEGIN
...
/*                IF ( (POS(:WK_ALLOWED_STATUS, 'PA', 0, 1)  >   -1) AND  */
                  IF ( (POSITION( 'PA', :WK_ALLOWED_STATUS, 1)  >  0) AND 
