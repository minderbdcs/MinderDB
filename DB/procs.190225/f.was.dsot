...
DECLARE VARIABLE WK_PALLET_QTYX VARCHAR(10);
DECLARE VARIABLE WK_PALLET_QTY INTEGER;
DECLARE VARIABLE WK_PALLET_OWNER VARCHAR(10);
DECLARE VARIABLE WK_CARTON_QTYX VARCHAR(10);
DECLARE VARIABLE WK_CARTON_QTY INTEGER;
DECLARE VARIABLE WK_SATCHEL_QTYX VARCHAR(10);
...
DECLARE VARIABLE WK_ADDRESS_LINE2         VARCHAR(100);
DECLARE VARIABLE WK_ADDRESS_LINE3         VARCHAR(100);
DECLARE VARIABLE WK_ADDRESS_LINE4         VARCHAR(100);
DECLARE VARIABLE WK_ADDRESS_LINE5         VARCHAR(100);
...
DECLARE VARIABLE WK_PA_ADDRESS_LINE2         VARCHAR(100);
...
DECLARE VARIABLE WK_PS_AWB_CONSIGNMENT_NO VARCHAR(50);
...
DECLARE VARIABLE WK_PD_TEST_AWB_CONSIGNMENT_NO VARCHAR(50);
DECLARE VARIABLE WK_CARRIER_ONE_M3_WEIGHT DOUBLE PRECISION;
DECLARE VARIABLE WK_PID_WEIGHT DESPATCHED_WEIGHT;
DECLARE VARIABLE WK_PID_WEIGHT_NET NUMERIC(16,3);
DECLARE VARIABLE WK_PID_WEIGHT_PACK NUMERIC(16,3);
DECLARE VARIABLE WK_PID_WEIGHT_CONV NUMERIC(16,3);
DECLARE VARIABLE WK_PID_WEIGHT_REAL NUMERIC(16,3);
DECLARE VARIABLE WK_KG_CONV NUMERIC(16,3);
...
DECLARE VARIABLE WK_PD_ADDRESS_LINE1 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE2 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE3 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE4 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE5 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE6 VARCHAR(100);
DECLARE VARIABLE WK_PD_ADDRESS_LINE7 VARCHAR(100);
DECLARE VARIABLE WK_PD_LAST_LINE     INTEGER;
DECLARE VARIABLE WK_SO_FIRST_NAME    FIRST_NAME;
DECLARE VARIABLE WK_SO_LAST_NAME     LAST_NAME;
DECLARE VARIABLE WK_PI_DEL_TO_STORE_IN_HOUSE_NO EDI_LOCN_NO;
DECLARE VARIABLE WK_PI_DEL_TO_DC_IN_HOUSE_NO EDI_LOCN_NO;
DECLARE VARIABLE WK_PI_CUSTOMER_EDI_NO EDI_NO;
DECLARE VARIABLE WK_DEL_TO_STORE_IN_HOUSE_NO EDI_LOCN_NO;
DECLARE VARIABLE WK_DEL_TO_DC_IN_HOUSE_NO EDI_LOCN_NO;
DECLARE VARIABLE WK_DEL_TO_LABEL_NO PICK_LABEL_NO;
DECLARE VARIABLE WK_PIA_FIRST_NAME FIRST_NAME;
DECLARE VARIABLE WK_PIA_LAST_NAME LAST_NAME;
DECLARE VARIABLE WK_PIA_ADDRESS_LINE1 VARCHAR(100);
DECLARE VARIABLE WK_PIA_ADDRESS_LINE2 ADDRESS_LINE;
DECLARE VARIABLE WK_PIA_ADDRESS_LINE3 ADDRESS_LINE;
DECLARE VARIABLE WK_PIA_ADDRESS_LINE4 ADDRESS_LINE;
DECLARE VARIABLE WK_PIA_ADDRESS_LINE5 ADDRESS_LINE;
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'start of of  DSOT');
     ...
     WK_PRINTER_IP               = NULL;
     WK_CITY                     = NULL; 
     WK_ADDRESS_LINE1            = NULL;  
     WK_ADDRESS_LINE2            = NULL;  
     WK_ADDRESS_LINE3            = NULL;  
     WK_ADDRESS_LINE4            = NULL;  
     WK_ADDRESS_LINE5            = NULL;  
     WK_SO_FIRST_NAME            = NULL;
     WK_SO_LAST_NAME             = NULL;
     WK_PI_DEL_TO_STORE_IN_HOUSE_NO = NULL;
     WK_PI_DEL_TO_DC_IN_HOUSE_NO = NULL;
     WK_PI_CUSTOMER_EDI_NO       = NULL;
     WK_DEL_TO_STORE_IN_HOUSE_NO = NULL;
     WK_DEL_TO_DC_IN_HOUSE_NO = NULL;
     WK_DEL_TO_LABEL_NO = NULL;

     WK_BUFFER = 'object ' || WK_OBJECT;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'sublocn ' || WK_CARRIER;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'wh ' || WK_WH_ID;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'locn ' || WK_LOCN_ID;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'qty ' || WK_LABEL_QTY;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'record ' || WK_RECORD;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     WK_BUFFER = 'class ' || WK_CLASS;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);

     WK_BUFFER = 'carrier ' || WK_CARRIER || ' account ' || WK_ACCOUNT || ' class ' || WK_CLASS || ' qty ' || WK_LABEL_QTY || ' wh_id ' || WK_WH_ID || ' locn ' || WK_LOCN_ID || ' record ' || WK_RECORD;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_SO);
        SELECT PICK_ORDER.P_POST_CODE, 
               ...
               UPPER(PICK_ORDER.S_ADDRESS_LINE1),
               UPPER(PICK_ORDER.S_ADDRESS_LINE2),
               PICK_ORDER.S_CITY,
               PICK_ORDER.S_SUBURB,
               PICK_ORDER.S_STATE,
               ...
               UPPER(PICK_ORDER.P_SUBURB), 
               UPPER(PICK_ORDER.P_ADDRESS_LINE1),  
               UPPER(PICK_ORDER.P_ADDRESS_LINE2),  
               UPPER(PICK_ORDER.P_ADDRESS_LINE3),  
               UPPER(PICK_ORDER.P_ADDRESS_LINE4),  
               UPPER(PICK_ORDER.P_ADDRESS_LINE5),  
               UPPER(PICK_ORDER.P_FIRST_NAME),
               UPPER(PICK_ORDER.P_LAST_NAME)
        FROM PICK_ORDER
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
/*
     WK_PALLET_QTYX = SUBSTR(WK_REF,1,4);   WK_PALLET_OWNER = SUBSTR(WK_REF,5,14);
     WK_CARTON_QTYX = SUBSTR(WK_REF,15,18); WK_SATCHEL_QTYX = SUBSTR(WK_REF,19,22);
     WK_WEIGHT_X = SUBSTR(WK_REF,23,27);    WK_VOLUME_X = SUBSTR(WK_REF,28,32);
     WK_PAYER = SUBSTR(WK_REF,33,33);       WK_LABEL_TYPE = SUBSTR(WK_REF,34,34);
     WK_SERVICE_TYPE = SUBSTR(WK_REF,35,37); WK_PACK_TYPE = SUBSTR(WK_REF,39,39);
     WK_PRINTER = 'P' || SUBSTR(WK_REF,40,40); WK_SERVICE_RECORD_X =  SUBSTR(WK_REF,41,50);
*/
     WK_PALLET_QTYX = SUBSTRING(:WK_REF FROM 1 FOR 4);
     WK_PALLET_OWNER = SUBSTRING(:WK_REF FROM 5 FOR 10);
     WK_CARTON_QTYX = SUBSTRING(:WK_REF FROM 15 FOR 4);
     WK_SATCHEL_QTYX = SUBSTRING(:WK_REF FROM 19 FOR 4);
     WK_WEIGHT_X = SUBSTRING(:WK_REF FROM 23 FOR 5);
     WK_VOLUME_X = SUBSTRING(:WK_REF FROM 28 FOR 5);
     WK_PAYER = SUBSTRING(:WK_REF FROM 33 FOR 1);
     WK_LABEL_TYPE = SUBSTRING(:WK_REF FROM 34 FOR 1);
     WK_SERVICE_TYPE = SUBSTRING(:WK_REF FROM 35 FOR 3);
     WK_PACK_TYPE = SUBSTRING(:WK_REF FROM 39 FOR 1);
     WK_PRINTER = 'P' || SUBSTRING(:WK_REF FROM 40 FOR 1);
     WK_SERVICE_RECORD_X = SUBSTRING(:WK_REF FROM 41 FOR 10);
/*
     WK_REF2 = SUBSTR(WK_REF, 51, STRLEN(WK_REF));
*/
     WK_REF2 = NEW.REFERENCE;
     SELECT WK_FIELD_TEXT FROM GET_REFERENCE_FIELD4 (:WK_REF2, 3, '|' ) INTO WK_CONSIGNMENT ;
     WK_BUFFER = 'passed connote [' || :WK_CONSIGNMENT || ']';
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     SELECT WK_FIELD_TEXT FROM GET_REFERENCE_FIELD4 (:WK_REF2, 4, '|' ) INTO WK_SERVICE_TYPE ;
     WK_CONSIGNMENT = ALLTRIM(WK_CONSIGNMENT);
     WK_CONSIGNMENT_PASSED = ALLTRIM(WK_CONSIGNMENT);
     WK_BUFFER = 'connote [' || WK_CONSIGNMENT || ']';
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);

     SELECT WK_FIELD_TEXT FROM GET_REFERENCE_FIELD4 (:WK_REF2, 5, '|' ) INTO WK_DEL_TO_DC_IN_HOUSE_NO ;
     WK_DEL_TO_DC_IN_HOUSE_NO = ALLTRIM(WK_DEL_TO_DC_IN_HOUSE_NO);
     SELECT WK_FIELD_TEXT FROM GET_REFERENCE_FIELD4 (:WK_REF2, 6, '|' ) INTO WK_DEL_TO_STORE_IN_HOUSE_NO ;
     WK_DEL_TO_STORE_IN_HOUSE_NO = ALLTRIM(WK_DEL_TO_STORE_IN_HOUSE_NO);
     IF (WK_DEL_TO_DC_IN_HOUSE_NO = '') THEN
     BEGIN
        WK_DEL_TO_DC_IN_HOUSE_NO = NULL;
     END
     IF (WK_DEL_TO_STORE_IN_HOUSE_NO = '') THEN
     BEGIN
        WK_DEL_TO_STORE_IN_HOUSE_NO = NULL;
     END
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_SO_COMPANY);
     ...
     SELECT PA_FIRST_NAME, PA_LAST_NAME, PA_ADDRESS_LINE1, PA_ADDRESS_LINE2,
            PA_CITY, PA_SUBURB, PA_STATE, PA_POST_CODE, PA_COUNTRY, PA_EMAIL,
            PA_PHONE_NO              
     FROM ADDRESS_DESPATCH_FROM (
        :WK_SO_COMPANY, :WK_SO_SEND_FIRST_NAME, :WK_SO_SEND_LAST_NAME,
        :WK_SO_SEND_ADDRESS_LINE1, :WK_SO_SEND_ADDRESS_LINE2, :WK_SO_SEND_CITY,
        :WK_SO_SEND_SUBURB, :WK_SO_SEND_STATE, :WK_SO_SEND_POST_CODE,
        :WK_SO_SEND_COUNTRY)
     INTO WK_PA_FIRST_NAME, WK_PA_LAST_NAME, WK_PA_ADDRESS_LINE1  , WK_PA_ADDRESS_LINE2  , WK_PA_CITY  ,
          WK_PA_SUBURB  , WK_PA_STATE  , WK_PA_POST_CODE  , WK_PA_COUNTRY, WK_PA_EMAIL,
          WK_PA_PHONE_NO  ;

     /* ================================================================== */
     SELECT ADDRESS_LINE1, ADDRESS_LINE2, ADDRESS_LINE3,
        ADDRESS_LINE4, ADDRESS_LINE5, ADDRESS_LINE6,
        ADDRESS_LINE7
     FROM ADDRESS_DESPATCH_BUBBLE (
	:WK_SO_FIRST_NAME, :WK_SO_LAST_NAME,
        :WK_POSTCODE , :WK_SUBURB , :WK_CITY   ,
        :WK_STATE   , :WK_COUNTRY , :WK_ADDRESS_LINE1 ,
        :WK_ADDRESS_LINE2 , :WK_ADDRESS_LINE3 , :WK_ADDRESS_LINE4 , :WK_ADDRESS_LINE5 )
     INTO WK_PD_ADDRESS_LINE1  , WK_PD_ADDRESS_LINE2  , WK_PD_ADDRESS_LINE3  ,
          WK_PD_ADDRESS_LINE4  , WK_PD_ADDRESS_LINE5  , WK_PD_ADDRESS_LINE6  ,
          WK_PD_ADDRESS_LINE7  ;
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_BUFFER);
     ...
     WK_BUFFER = 'pallets2 ' || WK_PALLET_QTY || ' cartons ' || WK_CARTON_QTY || ' satchels ' || WK_SATCHEL_QTY || ' weight ' || WK_WEIGHT || ' volume ' || WK_VOLUME|| ' sender ' || WK_SENDER_ACCT || ' receiver ' || WK_RECEIVER_ACCT || ' payer ' || WK_PAYER ;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_BUFFER);
...
/* select for passed dc and store here
   have to allow fields not being passed - forces the or's
*/
   AND ( PICK_ITEM.PS_DEL_TO_DC_IN_HOUSE_NO = :WK_DEL_TO_DC_IN_HOUSE_NO
   OR    :WK_DEL_TO_DC_IN_HOUSE_NO IS NULL)
   AND ( PICK_ITEM.PS_DEL_TO_STORE_IN_HOUSE_NO = :WK_DEL_TO_STORE_IN_HOUSE_NO
   OR    :WK_DEL_TO_STORE_IN_HOUSE_NO IS NULL)
           INTO :WK_DETAIL_ID, :WK_SSN_ID, :WK_PI_LABEL_NO,
           :WK_PI_DEL_TO_STORE_IN_HOUSE_NO,
           :WK_PI_DEL_TO_DC_IN_HOUSE_NO,
           :WK_PI_CUSTOMER_EDI_NO
           DO
           BEGIN
              IF (WK_DEL_TO_DC_IN_HOUSE_NO IS NULL) THEN
              BEGIN
                 WK_DEL_TO_DC_IN_HOUSE_NO = WK_PI_DEL_TO_DC_IN_HOUSE_NO;
              END
              IF (WK_DEL_TO_STORE_IN_HOUSE_NO IS NULL) THEN
              BEGIN
                 WK_DEL_TO_STORE_IN_HOUSE_NO = WK_PI_DEL_TO_STORE_IN_HOUSE_NO;
                 WK_DEL_TO_LABEL_NO = WK_PI_LABEL_NO;
              END
              WK_FOUND = 0;
        ...
        END
     END
     IF (WK_DEL_TO_LABEL_NO IS NOT NULL) THEN
     BEGIN
        /* get the address from the pick_item_address */
        SELECT FIRST 1 PIA_FIRST_NAME , PIA_LAST_NAME , PIA_SUBURB ,
             PIA_CITY , PIA_STATE , PIA_POST_CODE , PIA_COUNTRY ,
             PIA_ADDRESS_LINE1 , PIA_ADDRESS_LINE2 , PIA_ADDRESS_LINE3 ,
             PIA_ADDRESS_LINE4 , PIA_ADDRESS_LINE5  
        FROM PICK_ITEM_ADDRESS
        WHERE PICK_ORDER = :WK_SO
        AND   PICK_LABEL_NO = :WK_DEL_TO_LABEL_NO
        AND PIA_ADDR_TYPE = 'P'
        INTO 
         WK_PIA_FIRST_NAME , WK_PIA_LAST_NAME , WK_CITY ,
         WK_SUBURB , WK_STATE , WK_COUNTRY ,
         WK_POSTCODE , WK_PIA_ADDRESS_LINE1 , WK_PIA_ADDRESS_LINE2 ,
         WK_PIA_ADDRESS_LINE3 , WK_PIA_ADDRESS_LINE4 , WK_PIA_ADDRESS_LINE5 ;
        IF (COALESCE(WK_PIA_FIRST_NAME,'') = '') THEN 
        BEGIN
           WK_PIA_FIRST_NAME = WK_SO_FIRST_NAME;
           WK_PIA_LAST_NAME = WK_SO_LAST_NAME;
        END
        SELECT ADDRESS_LINE1, ADDRESS_LINE2, ADDRESS_LINE3,
        ADDRESS_LINE4, ADDRESS_LINE5, ADDRESS_LINE6,
        ADDRESS_LINE7
        FROM ADDRESS_DESPATCH_BUBBLE (
	:WK_PIA_FIRST_NAME, :WK_PIA_LAST_NAME,
        :WK_POSTCODE , :WK_SUBURB , :WK_CITY ,
        :WK_STATE   , :WK_COUNTRY , :WK_PIA_ADDRESS_LINE1 ,
        :WK_PIA_ADDRESS_LINE2 , :WK_PIA_ADDRESS_LINE3 , :WK_PIA_ADDRESS_LINE4 , :WK_PIA_ADDRESS_LINE5 )
        INTO WK_PD_ADDRESS_LINE1  , WK_PD_ADDRESS_LINE2  , WK_PD_ADDRESS_LINE3  ,
          WK_PD_ADDRESS_LINE4  , WK_PD_ADDRESS_LINE5  , WK_PD_ADDRESS_LINE6  ,
          WK_PD_ADDRESS_LINE7 ;
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     ...
     END
     WK_BUFFER = 'despatch [' || WK_DESPATCH_ID || ']';
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     /* create pick_despatch */
     WK_FOUND = 0;
     BEGIN
        UPDATE OR INSERT INTO PICK_DESPATCH
           ...
           PICKD_MANIFEST_ID,
           PD_SEND_TO_ADDR1, PD_SEND_TO_ADDR2, PD_SEND_TO_ADDR3, 
           PD_SEND_TO_ADDR4, PD_SEND_TO_ADDR5, PD_SEND_TO_ADDR6, 
           PD_SEND_TO_ADDR7 , PS_DEL_TO_DC_IN_HOUSE_NO, PS_DEL_TO_STORE_IN_HOUSE_NO, PS_CUSTOMER_EDI_NO)
           VALUES (
           ...
           NULL, /* no manifest yet */
           :WK_PD_ADDRESS_LINE1, :WK_PD_ADDRESS_LINE2, :WK_PD_ADDRESS_LINE3, 
           :WK_PD_ADDRESS_LINE4, :WK_PD_ADDRESS_LINE5, :WK_PD_ADDRESS_LINE6, 
           :WK_PD_ADDRESS_LINE7 , :WK_DEL_TO_DC_IN_HOUSE_NO, :WK_DEL_TO_STORE_IN_HOUSE_NO, :WK_PI_CUSTOMER_EDI_NO);
     END
     WK_BUFFER = 'written despatch ';
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
     ...
     BEGIN
        ...
        AND PICK_ITEM_DETAIL.PICK_DETAIL_STATUS = 'DS'
        AND PICK_ITEM_DETAIL.QTY_PICKED >= 0
/* only work on this dc and stores records */
        AND COALESCE(PICK_ITEM.PS_DEL_TO_DC_IN_HOUSE_NO,'') = COALESCE(:WK_DEL_TO_DC_IN_HOUSE_NO,'')
        AND COALESCE(PICK_ITEM.PS_DEL_TO_STORE_IN_HOUSE_NO,'') = COALESCE(:WK_DEL_TO_STORE_IN_HOUSE_NO,'')
        INTO :WK_DETAIL_ID, :WK_SSN_ID, :WK_PI_LABEL_NO
        DO
        BEGIN
     WK_BUFFER = 'got pid to update ';
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, WK_BUFFER);
           /* get pack no to use */
           WK_PACK_USED = NULL;
           IF (WK_TRACK_PACK_2_PID = 'T') THEN
           BEGIN
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'track pids is T');
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pack_id usage');
           END
           ...
           BEGIN
               ...
               WHERE PICK_LABEL_NO = :WK_PI_LABEL_NO;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pick item no issns');
           END
...
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated pack_sscc');

           /* calc weight on this despatch id */
/* have to include the uom.to_standard_conv
to calc the value in kgs
so get to_standard_conv for uom KG
divide the result by the conv to get kgs
if the conv is null or zero use 1
*/
           WK_KG_CONV = 1;
           SELECT  UOM.TO_STANDARD_CONV  
            FROM  UOM WHERE  UOM.CODE = 'KG'
           INTO :WK_KG_CONV;
           IF (COALESCE(WK_KG_CONV,0) <= 0) THEN
           BEGIN
              WK_KG_CONV = 1;
           END
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'kg conv ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_KG_CONV);
           SELECT (PID.QTY_PICKED * PP.NET_WEIGHT), UOM.TO_STANDARD_CONV , 
            ...
            LEFT OUTER JOIN UOM ON PP.NET_WEIGHT_UOM = UOM.CODE
            
            WHERE PID.PICK_DETAIL_ID = :WK_DETAIL_ID
            INTO :WK_PID_WEIGHT_NET, :WK_PID_WEIGHT_CONV, :WK_PID_WEIGHT_PACK;
            IF (COALESCE(WK_PID_WEIGHT_NET, WK_PID_WEIGHT_PACK, 0) > 0) THEN
            BEGIN
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'got weight ext ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT_NET);
               /* convert this to standard */
               IF (COALESCE(WK_PID_WEIGHT_CONV,0) <= 0) THEN
               BEGIN
                  WK_PID_WEIGHT_CONV = 1;
               END
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'got weight conv ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT_CONV);
               WK_PID_WEIGHT_REAL = :WK_PID_WEIGHT_NET * WK_PID_WEIGHT_CONV ;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT_REAL);
               /* convert this to kgs */
               WK_PID_WEIGHT_REAL = :WK_PID_WEIGHT_REAL / WK_KG_CONV ;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT_REAL);
               WK_PID_WEIGHT_NET = :WK_PID_WEIGHT_REAL  ;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'new weight net as kgs ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT_NET);
               WK_PID_WEIGHT_REAL = :WK_PID_WEIGHT_PACK * WK_PID_WEIGHT_CONV ;
               /* convert this to kgs */
               WK_PID_WEIGHT_REAL = :WK_PID_WEIGHT_REAL / WK_KG_CONV ;
               WK_PID_WEIGHT_PACK = :WK_PID_WEIGHT_REAL  ;

               IF (COALESCE(WK_PID_WEIGHT_NET, WK_PID_WEIGHT_PACK,0 ) < 999999) THEN
               BEGIN
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'weight lt 999999 ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'weight  sum ');
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, :WK_PID_WEIGHT);
                  WK_PID_WEIGHT = WK_PID_WEIGHT + COALESCE(WK_PID_WEIGHT_NET, WK_PID_WEIGHT_PACK, 0) ;
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'done adding this weight to sum ');
               END
            END
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'updated wk_pid_weight');
        END
     END
     ELSE
     IF (WK_CLASS = 'L') THEN
     ...
     IF (WK_CLASS = 'S') THEN
     BEGIN
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'about to check or add extra pids for tracking');
        IF (WK_TRACK_PACK_2_PID = 'T') THEN
        BEGIN
        ...
        END
     END
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'past add extra pids for tracking');
     ...
     BEGIN
        ...
        UPDATE PICK_DESPATCH SET PICKD_WT_ACTUAL = :WK_PID_WEIGHT,
           ...
           WHERE DESPATCH_ID = :WK_DESPATCH_ID ;
     END
WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'past update despatch cube weights');
