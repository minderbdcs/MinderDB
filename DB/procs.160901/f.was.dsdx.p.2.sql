...
/* 06/09/2016 pick line status NF is used at TIFS
   to use partial despatch T
   partial pick T or F
   but treat this as only one delivery possible 
   so need a parameter somewhere to say whether to do this 
   use options record for DSDX NF 
*/
...
DECLARE VARIABLE WK_DO_EXPORT_DESPATCH CHAR(1);
DECLARE VARIABLE WK_OP_DO_NF VARCHAR(50);
...
*/
    ...
    END
    WK_OP_DO_NF = 'F';
    SELECT DESCRIPTION FROM OPTIONS WHERE GROUP_CODE = 'DSDX' AND CODE = 'NF'
    INTO :WK_OP_DO_NF;
    IF (WK_OP_DO_NF IS NULL) THEN
    BEGIN
       WK_OP_DO_NF = 'F';
    END
                ...
                BEGIN
                   ...
                   BEGIN
                      ...
                      BEGIN
                         ...
                         ELSE
                         BEGIN
                            /* partial despatch T and partial pick F ???????????? */
                            UPDATE PICK_ITEM SET PICK_LINE_STATUS = 'NF',
                                DEVICE_ID = NULL
                            WHERE PICK_LABEL_NO = :WK_LABEL;
                         END
