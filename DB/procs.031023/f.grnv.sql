ALTER PROCEDURE ADD_SSN_ISSN_GRNV (WH_ID VARCHAR(2) ,
LOCN_ID VARCHAR(10) ,
OBJECT VARCHAR(30) ,
...
DECLARE VARIABLE WK_PRINTER_2 CHAR(1);
...
BEGIN 
   ...
   WK_REF2_LAST = WK_REF_LEN - 1;
   WK_OLD_METHOD = 'N';
   IF (SUBSTR(REFERENCE,WK_REF2_LAST,WK_REF2_LAST) = '|') THEN
   BEGIN
      IF (SUBSTR(REFERENCE,WK_REF_LEN,WK_REF_LEN) >= '0' AND
          SUBSTR(REFERENCE,WK_REF_LEN,WK_REF_LEN) <= '9') THEN
      BEGIN      
         WK_OLD_METHOD = 'Y';
      END
   END
...
*/
   ...
   BEGIN
       ...
       BEGIN
          ...
          VALUES (:W_SSN_ID, :WH_ID, :LOCN_ID, :GRN, 'PA', 'NOW', :QTY, :QTY, 'NOW',:WK_LOADNO, 'NOW', :OBJECT,:WK_PROD_DESC,:WK_LOAD_LINE_NO);     
      END
   END
...
*/
   ...
   BEGIN
      ...
      IF (WK_A_PROD = 0) THEN
      BEGIN
         WK_FILENAME = WK_DIRECTORY || 'LOAD2.txt';
         WK_LABEL_LINE = DQUOTEDSTR(W_SSN_ID) || ',' ||
             DQUOTEDSTR(WK_LOADNO) || ',' ||
             DQUOTEDSTR(LABEL_NO);
         WK_RESULT = FILE_WRITELN(WK_FILENAME, WK_LABEL_LINE);
      ...
      IF (WK_A_PROD = 1) THEN
      BEGIN
         WK_FILENAME = WK_DIRECTORY || 'Product2.txt';
         WK_LABEL_LINE = DQUOTEDSTR(W_SSN_ID) || ',' ||
             DQUOTEDSTR(WK_LABEL_CURQTY ) || ',' ||
             DQUOTEDSTR(OBJECT) || ',' ||
             DQUOTEDSTR(WK_PROD_DESC) || ',' ||
