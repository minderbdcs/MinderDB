...
DECLARE VARIABLE WK_PS_AWB_CONSIGNMENT_NO AWB_CONSIGNMENT_NO;
...
DECLARE VARIABLE WK_PS_LAST_DEL_TO_DC_NO VARCHAR(20);
DECLARE VARIABLE WK_PS_LAST_DEL_TO_STORE_NO VARCHAR(20);
DECLARE VARIABLE WK_PS_LAST_ASN_ID          VARCHAR(20);
BEGIN
 ...
 BEGIN
  ...
  BEGIN
     ...
     WK_LOG_FILENAME = '/data/tmp/DSAS.log';
...
*/
     ...
     BEGIN
        ...
        WK_FILENAME3 = WK_FILENAME3 || '.csv3';
        WK_LEGACY_ID  = '';
        WK_PS_LAST_DEL_TO_DC_NO = '';
        WK_PS_LAST_DEL_TO_STORE_NO = '';
...
*/
        ...
        FOR SELECT PACK_SSCC.PS_SSCC, 
         ...
         /*        PACK_SSCC.PS_CARRIER_ID, */
                   ...
                   (CAST(PROD_PROFILE.DIMENSION_X AS DOUBLE PRECISION) *  CAST(PROD_PROFILE.DIMENSION_Y AS DOUBLE PRECISION) * CAST(PROD_PROFILE.DIMENSION_Z AS DOUBLE PRECISION)) ,
                   PACK_SSCC.RECORD_ID
            FROM PACK_SSCC
       ...
       /*     AND PACK_SSCC.PS_SSCC_STATUS IN ('DC','DX','CL','DN','NF') */
            WHERE PS_OUT_DESPATCH_ID = :WK_PD_ID
              ...
              AND PACK_SSCC.PS_SSCC_STATUS IN ('DC','DX','CL','DN','NF','CK')
              AND  (COALESCE(PS_QTY_SHIPPED,0) > 0) 
              ORDER BY PACK_SSCC.PS_DEL_TO_DC_NO, PACK_SSCC.PS_DEL_TO_STORE_NO
            INTO :WK_PS_SSCC_ID, 
                 :WK_PS_ASN_VENDOR_NO, 
     ...
     WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME,'read loop');
           /* ==============   here reset asn ==================== */
           IF (WK_PS_DEL_TO_DC_NO <>  WK_PS_LAST_DEL_TO_DC_NO) THEN
           BEGIN
              WK_PS_ASN_ID = '';
              WK_PS_LAST_DEL_TO_DC_NO = WK_PS_DEL_TO_DC_NO ;  
           END
           IF (WK_PS_DEL_TO_STORE_NO <>  WK_PS_LAST_DEL_TO_STORE_NO) THEN
           BEGIN
              WK_PS_ASN_ID = '';
              WK_PS_LAST_DEL_TO_STORE_NO = WK_PS_DEL_TO_STORE_NO ;  
           END
           ...
           BEGIN
              ...
              WK_PS_ASN_ID = WK_LEGACY_ID ;
           END
           /* =========== write transaction work to save  asn no with packs record id ============ */
          INSERT INTO TRANSACTIONS_WORK(RECORD_ID, OBJECT, QTY) VALUES(:WK_RECORD, :WK_PS_ASN_ID, :WK_PS_RECORD_ID);

           /* create csv record */
           IF (COALESCE(WK_PS_DESPATCHED_DATE,'') = '') THEN
           BEGIN
              /* WK_PS_DESPATCHED_DATE = MER_YEAR('NOW') ||  LPAD(MER_MONTH('NOW'),'0',2) ||  LPAD(MER_DAY('NOW'),'0',2) ;  */
              WK_PS_DESPATCHED_DATE = EXTRACT(YEAR FROM CURRENT_TIMESTAMP(0)) ||  VLPAD(EXTRACT(MONTH FROM CURRENT_TIMESTAMP(0)),'0',2) ||  VLPAD(EXTRACT(DAY FROM CURRENT_TIMESTAMP(0)),'0',2) ;  
           END
           ...
           BEGIN
              ...
              WK_PS_PICK_ORDER_DATE = EXTRACT(YEAR FROM :WK_PO_CREATE_DATE) ||  VLPAD(EXTRACT(MONTH FROM :WK_PO_CREATE_DATE),'0',2) ||  VLPAD(EXTRACT(DAY FROM :WK_PO_CREATE_DATE),'0',2) ;  
           END
...
*/
     ...
     WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'after date');
           ...
           WK_PO_EXPORTED_DATE = EXTRACT(YEAR FROM CURRENT_TIMESTAMP(0)) ||  VLPAD(EXTRACT(MONTH FROM CURRENT_TIMESTAMP(0)),'0',2) ||  VLPAD(EXTRACT(DAY FROM CURRENT_TIMESTAMP(0)),'0',2) ;  
...
/* check that this is a float */
     ...
     WK_LOG_RESULT = FILE_WRITELN(WK_LOG_FILENAME, 'end   loop');
        IF (WK_IS_OK = 0) THEN
        BEGIN
           /* ===================== here we apply the asn to the pack_sscc  ======================= */
           /* PS_SSCC_STATUS = 'DC', */
...
/*
       ...
       -*  WHERE PS_DESPATCH_ID = :WK_PD_ID *-
           WHERE PS_OUT_DESPATCH_ID = :WK_PD_ID
           AND PS_EDI_ASN IS NULL
           AND PS_SSCC_STATUS IN ('DC','DX','CL','NF','CK')
           AND  (COALESCE(PS_QTY_SHIPPED,0) > 0);
           WK_ERROR_TEXT = 'PACK_SSCC.PS_EDI_ASN=' || :WK_PS_ASN_ID || '|';
*/
           /* for the saved asns and packs  */
           WK_PS_LAST_ASN_ID  = '';
           FOR SELECT TRANSACTIONS_WORK.OBJECT, TRANSACTIONS_WORK.QTY 
               FROM TRANSACTIONS_WORK
               WHERE TRANSACTIONS_WORK.RECORD_ID = :WK_RECORD
               ORDER BY TRANSACTIONS_WORK.OBJECT
               INTO :WK_PS_ASN_ID, :WK_PS_RECORD_ID
           DO
           BEGIN
              UPDATE PACK_SSCC SET PS_EDI_ASN = :WK_PS_ASN_ID,
              PS_SSCC_STATUS = 'DN',
              PS_LAST_UPDATE_DATE = :WK_DATE,
              PS_LAST_UPDATE_BY = :WK_USER
              WHERE RECORD_ID  = :WK_PS_RECORD_ID
              AND   PS_OUT_DESPATCH_ID = :WK_PD_ID
              AND PS_EDI_ASN IS NULL
              AND PS_SSCC_STATUS IN ('DC','DX','CL','NF','CK')
              AND  (COALESCE(PS_QTY_SHIPPED,0) > 0);
              IF (WK_PS_ASN_ID  <> WK_PS_LAST_ASN_ID) THEN
              BEGIN
                 WK_ERROR_TEXT  = WK_ERROR_TEXT ||  'PACK_SSCC.PS_EDI_ASN=' || :WK_PS_ASN_ID || '|';
                 WK_PS_LAST_ASN_ID = WK_PS_ASN_ID;
              END
           END
...
*/
          ...
          /*  WHERE PS_DESPATCH_ID = :WK_PD_ID */
              ...
              AND PS_SSCC_STATUS IN ('DC','DX','CL','CK')
              AND  (COALESCE(PS_QTY_SHIPPED,0) > 0);
           /* WK_ERROR_TEXT = 'PACK_SSCC.PS_EDI_ASN=' || :WK_PS_ASN_ID || '|'; */
              IF (STRLEN(WK_ERROR_TEXT) <= 232) THEN 
              BEGIN
