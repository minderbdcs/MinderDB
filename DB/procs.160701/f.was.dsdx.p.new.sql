...
/*
CREATE OR ALTER PROCEDURE PC_DSDX (RECORD_ID INTEGER,
WH_ID CHAR(2) ,
LOCN_ID VARCHAR(10) ,
OBJECT VARCHAR(30) ,
TRN_TYPE VARCHAR(4) ,
...
SUB_LOCN VARCHAR(10) )
*/
/* 18/07/2014 add an options record for whether to create a file to start email sending of a js packing slip */
...
DECLARE VARIABLE WK_RT_URI      CODE_255;
DECLARE VARIABLE WK_RT_TYPE     CODE_TWO;
DECLARE VARIABLE WK_RT_USER_ID  CODE;
DECLARE VARIABLE WK_RT_PASSWD   CODE;
DECLARE VARIABLE WK_RT_URL      CODE_128;
                ...
                BEGIN
                   ...
                   */
                   ...
                   BEGIN
                      ...
                      INTO :WK_RT_URI; */
                      SELECT R1.REPORT_URI, R1.REPORT_TYPE, R2.RT_USER_ID, R2.RT_PASS_WORD , R2.RT_URL
                      FROM REPORTS  R1
                      JOIN REPORTS_TYPE R2 ON R1.REPORT_TYPE = R2.CODE
                      WHERE R1.REPORT_ID = :WK_CM_PS_REPORT_ID
                      INTO :WK_RT_URI, :WK_RT_TYPE, :WK_RT_USER_ID, :WK_RT_PASSWD , :WK_RT_URL;
                      /* have to get rt_url rt user and rt password from reports type and add as parms 9 10 11 */
                      /* if wk_cn_event_directory is null  then cannot write the file!!!! */
                      /* so add to new table mail_?????  which posts an event */
                      IF (WK_RT_URI IS NOT NULL) THEN
                      BEGIN
                         IF (WK_CN_DIRECTORY IS NULL) THEN
                         BEGIN
                            /* must write to another table */
                            /* say print_requests with prn_type = 'EMAIL' */
                         END
                         ELSE
                         BEGIN
                            /* write the file */
