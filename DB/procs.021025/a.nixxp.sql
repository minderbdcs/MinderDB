...
/*            IF (:TRN_TYPE <> 'NIUI') THEN */
		...
		IF (:TRN_TYPE = 'NITP') THEN  
	        ...
	        END
                ELSE IF (:TRN_TYPE = 'NIUI') THEN
                BEGIN
                   EXECUTE PROCEDURE UPDATE_SSN(:OBJECT, 'NIUI', :REFERENCE); /* Update Old SSNID */
/*                   EXECUTE PROCEDURE ADD_MASTER_DATA(:TRN_TYPE, :TRN_DATE, :REFERENCE);   might want to copy the old ssn record details */
                   strAUDIT_DESC = "Old SSN Code modified";
                END
