...
*/
   ...
   /*           UPDATE PICK_ORDER SET PICK_PRIORITY = :WK_MOVE_PRIORITY,PICK_STATUS = 'HD'
        ...
        BEGIN
           ...
           WK_WORK_ORDER_QTY = 0;
           WK_WORK_PICKED_QTY = 0;
           SELECT PICK_ITEM.PICK_ORDER, PICK_ORDER.PARTIAL_PICK_ALLOWED, PICK_ORDER.WH_ID, PICK_ORDER.COMPANY_ID, PICK_ORDER.OTHER2
               FROM PICK_ITEM
               JOIN PICK_ORDER ON PICK_ORDER.PICK_ORDER = PICK_ITEM.PICK_ORDER
               WHERE PICK_ITEM.PICK_LABEL_NO = :WK_PI_LABEL
               AND PICK_ITEM.PICK_LINE_STATUS IN ('OP','UP')
               AND PICK_ORDER.PICK_STATUS IN ('OP','DA')
               INTO :WK_ORDER, :WK_PARTIAL_ORDER, WK_PO_WH_ID, :WK_PO_COMPANY_ID, :WK_PO_SALE_CHANNEL ;
              ...
              BEGIN
                 ...
                 BEGIN
                    ...
                    SELECT SUM( ISSN.CURRENT_QTY ) 
                          FROM ISSN
                          WHERE ISSN.PROD_ID = :WK_WORK_PROD
                          AND (V4POS(:WK_PO_COMPANY_ID,ISSN.COMPANY_ID,0,1) > -1)
        ...
        BEGIN
           ...
           BEGIN
              ...
              BEGIN
                 ...
                 END
              END
              SELECT WH_ID, COMPANY_ID, OTHER2
              FROM PICK_ORDER
              WHERE PICK_ORDER.PICK_ORDER = :WK_ORDER
              INTO :WK_PO_WH_ID, :WK_PO_COMPANY_ID, :WK_PO_SALE_CHANNEL;
              IF (WK_PO_WH_ID IS NULL) THEN
              BEGIN
                 WK_PO_WH_ID = 'ALL';
              END
