...
DECLARE VARIABLE WK_P_ORDER_WEIGHT DECIMAL( 5, 3);
...
DECLARE VARIABLE WK_P_NET_WEIGHT DECIMAL(4, 3);
DECLARE VARIABLE WK_P_INNER_WEIGHT DECIMAL(4, 3);
...
DECLARE VARIABLE WK_P_ISSUE_PER_PALLET       INTEGER;
DECLARE VARIABLE WK_P_ISSUE_PER_OUTER_CARTON INTEGER;
BEGIN
   BEGIN
   ...
   ELSE
   BEGIN
      /* SELECT COMPANY_ID FROM CONTROL INTO :WK_P_COMPANY_ID; */
      IF (WK_P_COMPANY_ID IS NULL) THEN
      BEGIN
         WK_P_COMPANY_ID = 'ALL';
      END
   END
   ...
   WK_P_OTHER2 = NULL;
   WK_P_OTHER3 = NULL;
   WK_P_ISSUE_PER_PALLET = NULL;
   WK_P_ISSUE_PER_OUTER_CARTON = NULL;
...
ISSUE_PER_INNER_UNIT , ORDER_WEIGHT , NET_WEIGHT_UOM , ORDER_WEIGHT_UOM , ISSUE ,
...
OTHER1 , OTHER2 , OTHER3 , ISSUE_PER_PALLET, ISSUE_PER_OUTER_CARTON
   FROM PROD_PROFILE
   WHERE PROD_ID = :WK_P_PROD_ID
   AND COMPANY_ID = :WK_P_COMPANY_ID
   INTO
   :WK_FOUND,
WK_P_SHORT_DESC ,
WK_P_LONG_DESC ,
...
WK_P_OTHER2 ,
WK_P_OTHER3 ,
WK_P_ISSUE_PER_PALLET ,
WK_P_ISSUE_PER_OUTER_CARTON ;
...
*/
   ...
   BEGIN
      WK_P_URL = NEW.P_URL ;
   END
   IF ((NEW.P_ISSUE_PER_PALLET  IS NOT NULL) AND (NEW.P_ISSUE_PER_PALLET <> 0)) THEN
   BEGIN
      WK_P_ISSUE_PER_PALLET = NEW.P_ISSUE_PER_PALLET ;
   END
   IF ((NEW.P_ISSUE_PER_OUTER_CARTON  IS NOT NULL) AND (NEW.P_ISSUE_PER_OUTER_CARTON <> 0)) THEN
   BEGIN
      WK_P_ISSUE_PER_OUTER_CARTON = NEW.P_ISSUE_PER_OUTER_CARTON ;
   END
...
:WK_P_SALE_PRICE , :WK_P_URL , :WK_P_OTHER1 , :WK_P_OTHER2 , :WK_P_OTHER3, :WK_P_ISSUE_PER_PALLET, :WK_P_ISSUE_PER_OUTER_CARTON );
   END
   ELSE
   BEGIN
...
PROD_RETRIEVE_STATUS = :WK_P_PROD_RETRIEVE_STATUS ,  
...
ISSUE_PER_PALLET = :WK_P_ISSUE_PER_PALLET, ISSUE_PER_OUTER_CARTON = :WK_P_ISSUE_PER_OUTER_CARTON
         WHERE PROD_ID   = :WK_P_PROD_ID 
         AND COMPANY_ID = :WK_P_COMPANY_ID   ;
   ...
   BEGIN
      IF (WK_G_PROD_GS1_EXTENSION IS NULL) THEN
      BEGIN
         WK_G_PROD_GS1_EXTENSION = '0';
      END
