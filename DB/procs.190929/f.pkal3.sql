...
DECLARE VARIABLE WK_WORK_LABEL PICK_LABEL_NO;
 ...
 BEGIN
  ...
  BEGIN
     ...
     BEGIN
        ...
        /* cannot trust the product or who for  in the reference - use the pick_label and sys_equip for this */
        WK_CN_PRIORITY = 0;
...
*/
        ...
        BEGIN
           ...
           BEGIN
              ...
              BEGIN
                 WK_WORK_PROD = TRIM(WK_WORK_PROD);
/*               IF (STRLEN(WK_WORK_PROD) = 0) THEN */
                 IF (CHAR_LENGTH(WK_WORK_PROD) = 0) THEN
                 BEGIN
                    WK_WORK_PROD = NULL;
                 END
              END
...
/*                     IF (LEN(WK_PROD_OK) < 53) THEN */
                       IF (CHAR_LENGTH(WK_PROD_OK) < 53) THEN
                       BEGIN
        ...
        BEGIN
           ...
           WK_WORK_ORDER_QTY = 0;
           WK_WORK_PICKED_QTY = 0;
           SELECT PICK_ITEM.PICK_ORDER, PICK_ORDER.PARTIAL_PICK_ALLOWED, PICK_ORDER.WH_ID, PICK_ORDER.COMPANY_ID, PICK_ORDER.OTHER2, PICK_ORDER.ALTERNATE_COMPANYS
               FROM PICK_ITEM
               JOIN PICK_ORDER ON PICK_ORDER.PICK_ORDER = PICK_ITEM.PICK_ORDER
               WHERE PICK_ITEM.PICK_LABEL_NO = :WK_PI_LABEL
               AND PICK_ITEM.PICK_LINE_STATUS IN ('OP','UP')
               AND PICK_ORDER.PICK_STATUS IN ('OP','DA')
               INTO :WK_ORDER, :WK_PARTIAL_ORDER, WK_PO_WH_ID, :WK_PO_COMPANY_ID, :WK_PO_SALE_CHANNEL, :WK_PO_ALTERNATE_COMPANYS ;
              ...
              BEGIN
                 ...
                 BEGIN
                    ...
                    SELECT SUM( ISSN.CURRENT_QTY ) 
                          FROM ISSN
                          WHERE ISSN.PROD_ID = :WK_WORK_PROD
                          AND (:WK_PO_COMPANY_ID = ISSN.COMPANY_ID
                          OR   (POSITION( ISSN.COMPANY_ID, :WK_PO_ALTERNATE_COMPANYS, 1) > 0))
                          AND (POSITION( ISSN.WH_ID, :WK_PO_WH_ID, 1) > 0)
                          AND (POSITION( ISSN.ISSN_STATUS, :WK_IMPORTSTATUS, 1) > 0)
                          GROUP BY ISSN.PROD_ID
                       INTO :WK_AVAILABLE_QTY;
   /* need to limit by product and channel for reservation */
                    IF (WK_AVAILABLE_QTY IS NULL) THEN
                    BEGIN
                       WK_AVAILABLE_QTY = 0;
                    END
...
/*                  IF (LEN(WK_PROD_OK) < 53) THEN */
     ...
     BEGIN
        ...
        WK_PI_LABEL = NEW.OBJECT;
        WK_DATE = NEW.TRN_DATE;
/* this is used by direct delivery - so stock will be ok
   however have to reduce the prod_reservation available qty down
*/
        /* cannot trust the product or who for  in reference - use the pick_label and sys_equip for this */
        WK_CN_PRIORITY = 0;
        WK_BUFFER = '';
        SELECT CURRENT_PERSON, LAST_PERSON
        FROM SYS_EQUIP
...
/*               IF (STRLEN(WK_WORK_PROD) = 0) THEN */
                 ...
                 END
              END
              SELECT WH_ID, COMPANY_ID, OTHER2, ALTERNATE_COMPANYS
              FROM PICK_ORDER
              WHERE PICK_ORDER.PICK_ORDER = :WK_ORDER
              INTO :WK_PO_WH_ID, :WK_PO_COMPANY_ID, :WK_PO_SALE_CHANNEL, :WK_PO_ALTERNATE_COMPANYS;
              IF (WK_PO_WH_ID IS NULL) THEN
              BEGIN
                 WK_PO_WH_ID = 'ALL';
              END
              ...
              BEGIN
                 BEGIN
                    ...
                    IF (WK_DO_SALE_CHANNEL = 'T') THEN
                    BEGIN
                        dont need to check the stock available
                        since this is only performed in the direct delivery
                        so there will be stock
                    END
                    ================================================================== */
                    /* now must get qty allocated on order not picked */
                    WK_PICKED_QTY = 0;
                    ...
                    BEGIN
                       ...
                       UPDATE PICK_ORDER
                          SET PICK_STARTED = 'NOW'
                           WHERE PICK_ORDER = :WK_ORDER
                           AND PICK_STARTED IS NULL;
                        /* do psur to reduce available qty in reservation */
                        /* ================================================================== */
                       IF (WK_DO_SALE_CHANNEL = 'T') THEN
                       BEGIN
                             ...
                             BEGIN
                                ...
                                BEGIN
                                   ...
                                   EXECUTE PROCEDURE ADD_TRAN(
                                      ...
                                      /* product in the object field */
                                      /* channel code in the reference field */
                                      /* record_id passed in the reference field */
                                END
                             END
                          END
                       END
