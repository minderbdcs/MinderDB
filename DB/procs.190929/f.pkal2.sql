                 ...
                 BEGIN
                    ...
                    UPDATE PICK_ITEM 
                       SET USER_ID = :WK_WHO_FOR, 
                        ...
                        AND PICK_LINE_STATUS IN ('OP','UP')
                        AND PICK_ORDER_QTY = 0;
                    /* add to transactions archive records for each prod in the order */
                    FOR SELECT PROD_ID 
                       FROM PICK_ITEM
                       WHERE PICK_ORDER = :WK_ORDER
                       AND PICK_LINE_STATUS IN ('OP','UP')
                    ...
                    BEGIN
                       ...
                       VALUES ( NEW.WH_ID, NEW.LOCN_ID, NEW.OBJECT, 'PKAL', 'i', NEW.TRN_DATE, NEW.REFERENCE,
                       0, '', NEW.INSTANCE_ID, NEW.EXPORTED, NEW.SUB_LOCN_ID, NEW.INPUT_SOURCE,
                       NEW.PERSON_ID, NEW.DEVICE_ID, NEW.RECORD_ID ,:WK_WORK_PROD, :WK_PO_COMPANY_ID, :WK_ORDER );
                    END
                    UPDATE PICK_ITEM 
                       SET USER_ID = :WK_WHO_FOR, 
                           DEVICE_ID = :WK_DEVICE,
